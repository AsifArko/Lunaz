              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <span className={`block text-sm font-medium ${isSelected ? config.textColor : 'text-gray-700'}`}>
                      {config.label}
                    </span>
                    <span className="block text-xs text-gray-500">{config.description}</span>
                  </div>
                  {isSelected && (
                    <svg className={`w-4 h-4 shrink-0 ${config.textColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">The order you're looking for doesn't exist or has been removed.</p>
          <Link
            to="/orders"
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full ${currentStatusConfig.badgeBg} ${currentStatusConfig.badgeText}`}>
            ...
            >
              <div className="relative">
                ...
                <div 
                  ...
                  style={{ width: `calc(${(currentStatusIndex / (statusTimeline.length - 1)) * 100}% - 4rem)` }}
                />
                ...
                <div className="relative flex items-start justify-between">
                  {statusTimeline.map((status, index) => {
                    ...
                    return (
                      <div key={status} className="flex flex-col items-center" style={{ width: '20%' }}>
                        <div
            ...
            <div className="bg-red-50 border border-red-100 rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-red-600">This order has been cancelled and cannot be processed further.</p>
                  {order.notes && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-t border-b border-gray-100 bg-gray-50/50">
                    <th className="px-4 py-2 text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider">Product</th>
                    <th className="px-4 py-2 text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider">Variant</th>
                    <th className="px-4 py-2 text-center text-[10px] font-medium text-gray-500 uppercase tracking-wider">Qty</th>
                    <th className="px-4 py-2 text-right text-[10px] font-medium text-gray-500 uppercase tracking-wider">Price</th>
                    <th className="px-4 py-2 text-right text-[10px] font-medium text-gray-500 uppercase tracking-wider">Total</th>
                  </tr>
                </thead>
                    ...
                    <tr key={index} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs text-gray-700 truncate max-w-[180px]">{item.productName}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2">
                        {item.variantName ? (
                      ...
                      <td className="px-4 py-2 text-right">
                        <Price amount={item.unitPrice} currency={order.currency} className="text-xs text-gray-600" />
                      </td>
                      <td className="px-4 py-2 text-right">
                        <Price amount={item.total} currency={order.currency} className="text-xs font-medium text-gray-900" />
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          >
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <Price amount={order.subtotal} currency={order.currency} className="text-sm text-gray-900" />
              </div>
              <div className="flex justify-between items-center">
                ...
                {order.shippingAmount === 0 ? (
                  ...
                  <Price amount={order.shippingAmount} currency={order.currency} className="text-sm text-gray-900" />
                )}
              </div>
              {order.taxAmount != null && order.taxAmount > 0 && (
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Tax</span>
                  <Price amount={order.taxAmount} currency={order.currency} className="text-sm text-gray-900" />
                </div>
              )}
              <div className="border-t border-gray-200 pt-3 mt-3">
                <div className="flex justify-between items-center">
                  <span className="text-base font-semibold text-gray-900">Total</span>
                  <Price amount={order.total} currency={order.currency} className="text-lg font-bold text-gray-900" />
                </div>
              </div>
            </div>
          </SidebarCard>
