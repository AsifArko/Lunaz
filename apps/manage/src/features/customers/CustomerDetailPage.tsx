...
const statusConfig: Record<OrderStatus, {
  ...
  confirmed: { label: 'Confirmed', color: 'bg-blue-500', bgColor: 'bg-blue-50', textColor: 'text-blue-700' },
  processing: { label: 'Processing', color: 'bg-indigo-500', bgColor: 'bg-indigo-50', textColor: 'text-indigo-700' },
  shipped: { label: 'Shipped', color: 'bg-purple-500', bgColor: 'bg-purple-50', textColor: 'text-purple-700' },
  delivered: { label: 'Delivered', color: 'bg-emerald-500', bgColor: 'bg-emerald-50', textColor: 'text-emerald-700' },
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const ordersRes = await api<PaginatedResponse<Order>>(`/orders?userId=${id}&limit=100`, { token });
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">The customer you're looking for doesn't exist or has been removed.</p>
          <Link
            to="/customers"
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-t border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th className="px-4 py-2 text-center text-[10px] font-medium text-gray-500 uppercase tracking-wider">Items</th>
                      ...
                      {paginatedOrders.map((order) => {
                        ...
                        return (
                          <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-4 py-2">
                              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded ${status.bgColor} ${status.textColor}`}>
                                <span className={`w-1 h-1 rounded-full ${status.color}`} />
                                {status.label}
                              </span>
                            </td>
                            <td className="px-4 py-2 text-right">
                              <Price amount={order.total} currency={order.currency} className="text-xs font-medium text-gray-900" />
                            </td>
                            <td className="px-4 py-2">
        ...
        <div className="space-y-5">
          ...
          >
            <div className="space-y-4">
              <div>
                <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Full Name</p>
                <p className="text-sm font-medium text-gray-900">{customer.name}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Email Address</p>
                <p className="text-sm text-gray-700">{customer.email}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Member Since</p>
