            ...
            {statusOptions.map((option) => (
              ...
              >
                ...
                <div className="flex-1">
                  <span className={`block text-sm font-medium ${value === option.value ? option.textColor : 'text-gray-700'}`}>
                    {option.label}
                  </span>
                  <span className="block text-xs text-gray-500">{option.description}</span>
                </div>
                {value === option.value && (
                  <svg className={`w-4 h-4 shrink-0 ${option.textColor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                  </svg>
                )}
              </button>
...
function VariantItem({ 
  ...
  return (
    <div className="border border-gray-100 rounded-lg overflow-hidden bg-gray-50/30 hover:bg-gray-50/50 transition-colors">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {variant.sku && (
              <span className="ml-2 text-xs text-gray-400">SKU: {variant.sku}</span>
            )}
          </div>
        </div>
      ...
      <div className={`overflow-hidden transition-all duration-200 ${variant.isExpanded ? 'max-h-96' : 'max-h-0'}`}>
        <div className="px-4 pb-4 pt-1 border-t border-gray-100 bg-white">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Price Override</label>
              <input
                type="number"
                step="0.01"
...
export function ProductFormPage() {
  ...
  const addVariant = () => {
    ...
    setVariants((prev) => [...prev, { id: generateId(), name: '', sku: '', priceOverride: '', stock: '', isExpanded: true }]);
  };
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {isEdit ? 'Update product information and inventory' : 'Add a new product to your catalog'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Link
              ...
              <div className="space-y-5">
                ...
                <FormField label="URL Slug" required hint="URL-friendly identifier (auto-generated from name)">
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">/products/</span>
                    <input
                      type="text"
                      value={slug}
                ...
                <FormField label="Description" hint="Detailed product description (supports basic formatting)">
                  <textarea
                    value={description}
                    ...
                    {imageUrlsToAdd.map((url, index) => (
                      <div key={`url-${index}`} className="relative group aspect-square">
                        <img
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" fill="%23ccc"><rect width="96" height="96"/><text x="48" y="48" text-anchor="middle" dy=".3em" font-size="10">Invalid</text></svg>';
                          }}
                        />
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-colors ${
                      ...
                      <svg className={`w-6 h-6 ${isDragging ? 'text-indigo-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                  ...
                  <div className="pt-3 border-t border-gray-100">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-semibold text-gray-900">
                        {currencySymbols[currency]}{parseFloat(basePrice || '0').toFixed(2)}
                      </span>
                    </div>
                  </div>
                )}
