...
export function ProductDetailPage() {
  ...
  const allowBackorder = publicSettings?.allowBackorder ?? false;
  const isOutOfStock = stockStatus === 0;
  const canAddToCart = !isOutOfStock || allowBackorder;
                  ...
                  <div className="flex-1 min-w-[180px]">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {product.variants.map((variant) => {
                        ...
                        const variantSelectable = !variantOutOfStock || allowBackorder;
                        return (
                          <button
                            key={variant.id}
                            onClick={() => setSelectedVariant(variant)}
                            disabled={!variantSelectable}
