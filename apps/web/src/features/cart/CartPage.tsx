...
function isInsideDhaka(city: string | undefined): boolean {
  if (!city) return false;
  return city.toLowerCase().includes('dhaka');
}
...
export function CartPage() {
  ...
  const [guestCity, setGuestCity] = useState<'dhaka' | 'outside' | null>(null);

  // Fetch user's default address city when logged in
  useEffect(() => {
    ...
    api<User>('/users/me', { token })
      .then((user) => {
        ...
        setUserCity(defaultAddr?.city ?? null);
      })
      .catch(() => setUserCity(null));
  }, [isAuthenticated, token]);

  // Shipping: use address city (logged-in) or guest selection (guest)
  const { shippingAmount, showShippingBreakdown } = useMemo(() => {
    const insideRate = publicSettings?.shippingInsideDhaka ?? 100;
    const outsideRate = publicSettings?.shippingOutsideDhaka ?? 150;

    let isInside: boolean;
    if (isAuthenticated && userCity !== null) {
      isInside = isInsideDhaka(userCity);
    } else if (!isAuthenticated && guestCity) {
      isInside = guestCity === 'dhaka';
    } else {
      return { shippingAmount: 0, showShippingBreakdown: false };
    }

    const amount = isInside ? insideRate : outsideRate;
    return { shippingAmount: amount, showShippingBreakdown: true };
  }, [publicSettings, isAuthenticated, userCity, guestCity]);

  // Tax calculation when enableTax and displayTaxInCart are on
  const { taxAmount, showTaxBreakdown, taxLabel, taxRatePercent } = useMemo(() => {
    const enableTax = publicSettings?.enableTax ?? false;
    const displayTaxInCart = publicSettings?.displayTaxInCart ?? false;
    const taxRatePercentVal = publicSettings?.taxRate ?? 0;
    const taxRate = taxRatePercentVal / 100;
    const taxIncludedInPrices = publicSettings?.taxIncludedInPrices ?? false;

    if (!enableTax || !displayTaxInCart || taxRate <= 0) {
      return { taxAmount: 0, showTaxBreakdown: false, taxLabel: 'Tax', taxRatePercent: 0 };
    }

    let amount: number;
    if (taxIncludedInPrices) {
      // Tax is already in subtotal: extract it. subtotal = preTax * (1 + rate) => preTax = subtotal / (1 + rate)
      const preTax = subtotal / (1 + taxRate);
      amount = Math.round(subtotal - preTax);
    } else {
      amount = Math.round(subtotal * taxRate);
    }

    return {
      taxAmount: amount,
      showTaxBreakdown: true,
      taxLabel: publicSettings?.taxLabel ?? 'Tax',
      taxRatePercent: taxRatePercentVal,
    };
  }, [publicSettings, subtotal]);

  const estimatedTotal =
    subtotal +
    (showShippingBreakdown ? shippingAmount : 0) +
    (showTaxBreakdown && !publicSettings?.taxIncludedInPrices ? taxAmount : 0);
            ...
            <div className="lg:col-span-5">
              <div className="bg-gray-50 rounded-xl p-6 lg:p-8 sticky top-8">
                ...
                <div className="space-y-3 mb-6 pb-6 border-b border-gray-200">
                  ...
                  {!isAuthenticated && !guestCity && (
                    <div className="pt-2">
                      <label className="block text-xs font-medium text-gray-600 mb-1.5">
                        Select city for shipping estimate
                      </label>
                      <select
                        value=""
                        onChange={(e) =>
                          setGuestCity((e.target.value as 'dhaka' | 'outside') || null)
                        }
                        className="w-full h-9 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-gray-900 focus:border-transparent"
                      >
                  ...
                  {!isAuthenticated && guestCity && (
                    <div className="pt-1 flex items-center justify-between">
                      <span className="text-xs text-gray-500">
                        Shipping to: {guestCity === 'dhaka' ? 'Dhaka' : 'Outside Dhaka'}
                      </span>
                      <button
                        type="button"
                        onClick={() => setGuestCity(null)}
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">
                      {taxLabel}
                      {showTaxBreakdown && taxRatePercent > 0 && (
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600">Easy Refunds</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-gray-200">
                        <svg
